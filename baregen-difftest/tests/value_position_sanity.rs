...
#[baregen::coroutine(yield = u32, resume = u32)]
#[derive(Clone, serde::Serialize, serde::Deserialize)]
fn match_value_try_res(a0: u32) -> Result<u32, baregen_difftest::Err1> {
    let q0: Result<u32, baregen_difftest::Err2> = if a0 % 3u32 == 0u32 {
        Err(baregen_difftest::Err2(a0))
    } else {
        Ok(a0)
    };
...
fn match_value_try_res_ref(a0: u32) -> Result<u32, baregen_difftest::Err1> {
    let q0: Result<u32, baregen_difftest::Err2> = if a0 % 3u32 == 0u32 {
        Err(baregen_difftest::Err2(a0))
    } else {
        Ok(a0)
