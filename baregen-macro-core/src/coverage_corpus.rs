...
//! `baregen-difftest/build.rs`; each case module carries a `SOURCE`
//! string constant with the exact coroutine source. Set
//! `BAREGEN_MACRO_CORPUS` to a colon-separated list of such files;
//! without it, any `cases.rs` under the workspace `target/` build
//! output of baregen-difftest is used. The test skips (with a message)
...
/// Corpus files: explicit list from `BAREGEN_MACRO_CORPUS`, or every
/// difftest `cases.rs` found under the workspace target directory.
fn corpus_paths() -> Vec<PathBuf> {
    if let Ok(list) = std::env::var("BAREGEN_MACRO_CORPUS") {
...
fn expand_case(source: &str) -> Result<(), String> {
    ...
    let idx = item
        ...
        .ok_or("no #[baregen::coroutine] attribute in SOURCE")?;
...
fn expand_difftest_corpus() {
    ...
    if paths.is_empty() {
        eprintln!(
            "coverage_corpus: no corpus found (set BAREGEN_MACRO_CORPUS or build \
             baregen-difftest first); skipping"
        );
        return;
    }
