...
//!
//! Takes a pasted function with its `#[diapause::coroutine(...)]`
...
/// `coroutine` attribute attached (`coroutine` and `diapause::coroutine`
...
fn take_coroutine_attr(item: &mut syn::ItemFn) -> syn::Result<TokenStream> {
    let pos = item
        ...
        .ok_or_else(|| {
            syn::Error::new(
                item.sig.ident.span(),
                "no #[diapause::coroutine(...)] attribute on this function",
            )
        })?;
    ...
    match attr.meta {
        ...
        syn::Meta::NameValue(nv) => Err(syn::Error::new_spanned(
            nv,
            "expected #[diapause::coroutine(...)], not a name-value attribute",
        )),
    }
}
...
fn is_coroutine_attr(attr: &syn::Attribute) -> bool {
    ...
    match segments.len() {
        ...
        2 => segments[0].ident == "diapause" && segments[1].ident == "coroutine",
        _ => false,
    }
}
...
mod tests {
    ...
    fn attribute_without_args_expands() {
        let out = transform("#[diapause::coroutine]\nfn c() {\n    yield_!(());\n}");
        assert_eq!(out.errors, vec![]);
    }
    ...
    fn other_attributes_are_kept() {
        let out = transform(
            "#[diapause::coroutine(fingerprint)]\n\
             #[derive(Clone)]\n\
             fn c() {\n\
                 yield_!(());\n\
             }",
    ...
    fn missing_attribute_is_an_error() {
        ...
        assert!(err.message.contains("no #[diapause::coroutine"), "{err:?}");
    }
