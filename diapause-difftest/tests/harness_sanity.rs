...
#[diapause::coroutine(yield = u32, resume = u32)]
#[derive(Clone, serde::Serialize, serde::Deserialize)]
fn fallible_sum(n: u32) -> Result<u32, diapause_difftest::Err1> {
    let mut acc: u32 = 0u32;
    for i in 0u32..n {
        let r = yield_!(acc);
        let step: Result<u32, diapause_difftest::Err2> = if r.is_multiple_of(3u32) {
            Ok(r)
        } else {
            Err(diapause_difftest::Err2(r))
        };
        let x: u32 = step?;
        acc = acc.wrapping_add(i).wrapping_add(x);
    }
    if acc.is_multiple_of(5u32) {
        return Err(diapause_difftest::Err1(acc));
    }
    Ok(yield_!(acc))
}

fn fallible_sum_ref(n: u32) -> Result<u32, diapause_difftest::Err1> {
    let mut acc: u32 = 0u32;
    for i in 0u32..n {
        let r = yield_!(acc);
        let step: Result<u32, diapause_difftest::Err2> = if r.is_multiple_of(3u32) {
...
fn harness_detects_result_divergence() {
    fn wrong_ref(n: u32) -> Result<u32, diapause_difftest::Err1> {
        let mut acc: u32 = 0u32;
        for i in 0u32..n {
            let r = yield_!(acc);
            ...
            let step: Result<u32, diapause_difftest::Err2> = if r.is_multiple_of(3u32) {
                ...
                Err(diapause_difftest::Err2(r.wrapping_add(1u32)))
            };
            let x: u32 = step?;
            acc = acc.wrapping_add(i).wrapping_add(x);
        }
