...
pub fn render_case(idx: usize, case: &Case) -> String {
    ...
    let ret = match case.flavor {
        ...
        Flavor::ResultU32 => " -> Result<u32, diapause_difftest::Err1>",
    ...
    let attr = format!("#[diapause::coroutine(yield = {yield_ty}, resume = u32{fp})]");
...
proptest::proptest! {{
    #![proptest_config(diapause_difftest::proptest_config())]
    #[test]
    fn {name}(
        a0 in 0u32..16u32,
    ...
    match s {
        ...
        Stmt::LetResult { name, init } => {
            let init = match init {
                ...
                Err(e) => format!("Err(diapause_difftest::Err2({}))", expr(e)),
            };
            out.push_str(&format!(
                "{i}let {name}: Result<u32, diapause_difftest::Err2> = {init};\n"
            ));
        }
            ...
            match ctx.world {
                ...
                World::Reference => {
                    ...
                    match bind {
                        ...
                        DelegateBind::Res(name) => {
                            out.push_str(&format!(
                                "{i}let {name}: Result<u32, diapause_difftest::Err1> = {call};\n"
                            ));
                        }
                    }
                }
...
fn expr(e: &Expr) -> String {
    match e {
        ...
        Expr::PairField { x, y, second } => {
            ...
            format!(
                "(diapause_difftest::Pair {{ x: {}, y: {} }}.{field})",
                expr(x),
                expr(y)
            )
        }
