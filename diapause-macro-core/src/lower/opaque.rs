...
impl Lowerer {
    ...
    /// labeled block outside it is rewritten into a `__diapause_jump!`
...
impl OpaqueRewriter<'_> {
    ...
    fn goto_marker(
        ...
        syn::parse_quote_spanned!(span => __diapause_jump!(#k))
    }
    ...
    fn rewrite_break(&mut self, b: &syn::ExprBreak) -> Option<syn::Expr> {
        ...
        match dest {
            ...
            BreakDest::Return => {
                ...
                Some(syn::parse_quote_spanned!(span => __diapause_jump!(#k, #v)))
            }
        }
    }
